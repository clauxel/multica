...
import { loadDeploymentConfig, readConfiguredMulticaRepoRef } from './server-lib/deployment-config.mjs'
import {
...
const explicitEnvironmentPath = String(process.env.MULTICA_ENV_PATH ?? '').toLowerCase()
...
const deploymentConfigPath =
  process.env.MULTICA_CONFIG_PATH ?? join(projectRoot, 'multica.config.json')
...
const deploymentConsoleBaseUrl = process.env.MULTICA_CONSOLE_BASE_URL ?? 'https://console.multica.local'
const deploymentPublicBaseUrl = process.env.MULTICA_PUBLIC_BASE_URL ?? 'https://multica.local'
...
const guestUserEmail = 'guest@multica.local'
const guestUserName = 'Multica Guest'
...
const serverSeoPageMap = new Map([
  ...
  [
    '/compare/self-hosted-deployment',
    {
      title: 'Multica vs self-hosted deployment | Multica',
      description:
        'Compare Multica with self-hosted deployment for launch speed, operational overhead, repeat deployment work, and ongoing maintenance.',
      robots: 'index,follow',
    },
  ],
  [
    '/compare/manual-agent-deployment',
    {
      title: 'Multica vs manual agent deployment | Multica',
      description:
        'Compare Multica with manual agent deployment when the main question is whether to keep stitching setup, payment, and follow-up work together by hand.',
      robots: 'index,follow',
    },
  ],
  [
    ...
    {
      title: 'Deploy Multica for customer support workflows | Multica',
      description:
  ...
  [
    ...
    {
      title: 'Deploy Multica for daily briefings and internal updates | Multica',
      description:
  ...
  [
    ...
    {
      title: 'Deploy Multica for ops automation and routine follow-up | Multica',
      description:
  ...
  [
    '/console',
    {
      ...
      description:
        'Track Multica agent orders, deployments, upgrades, and account operations inside the console.',
      robots: 'noindex,nofollow',
    },
  ],
  [
...
async function resolveMulticaProxyTarget(request, overrideSessionId, overridePathSuffix) {
  ...
  if (!session) {
    throw new HttpError(410, 'Multica console session expired. Reopen it from the dashboard.')
  }
  ...
  if (claw?.runtime_state === 'stopped') {
    throw new HttpError(400, 'Multica is stopped. Start a new deployment or uninstall this instance first.')
  }
...
async function fetchMulticaProxyUpstream(target, request, method, body) {
  ...
  throw lastError ?? new Error('Multica console upstream request failed.')
}
...
async function handleMulticaConsoleProxyRequest(request, response) {
  ...
  try {
    ...
    throw new HttpError(502, 'Multica console is temporarily unreachable.')
  }
...
async function handleMulticaConsoleProxyUpgrade(request, socket, head) {
  ...
  try {
    ...
    try {
      target = await resolveMulticaProxyTarget(request)
      if (!target) {
        throw new HttpError(404, 'Multica console session route not found.')
      }
    } catch (error) {
  ...
  const startAttempt = (attemptIndex) => {
    ...
    upstreamRequest.setTimeout(5000, () => {
      upstreamRequest.destroy(new Error('Multica websocket upstream timed out.'))
    })
...
async function createDeploymentForOrder(order, triggerMode) {
  ...
  await updateOrderDeploymentStatus(
    ...
    'queued',
    triggerMode === 'manual'
      ? 'Manual deployment trigger accepted. A new Multica is queued.'
      : 'Payment confirmed. Deployment is queued and will begin shortly.',
  )
...
async function updateAgentUpgradeState(clawId, patch) {
  ...
  if (!current) {
    throw new HttpError(404, 'Multica instance not found.')
  }
...
async function runCommand(command, args, options = {}) {
  return await new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        reject(new Error(stderr.trim() || `Deployment command exited with code ${code}.`))
        return
      }
...
async function executeDeployment(context) {
  ...
  if (!allowSimulatedDeployment) {
    throw new HttpError(503, 'Deployment runner is not configured.')
  }
...
async function processDeployment(deploymentId) {
  ...
  try {
    ...
    await updateDeploymentRecord(deploymentId, {
      status: 'provisioning',
      progress: 20,
      lastMessage: 'Payment captured. Preparing deployment workspace.',
      startedAt: nowIso(),
      updatedAt: nowIso(),
    })
    await updateOrderDeploymentStatus(order.id, 'provisioning', 'Deployment started. Preparing your Multica workspace.')
    ...
    await updateDeploymentRecord(deploymentId, {
      progress: 55,
      lastMessage: 'Connecting to the deployment target and applying Multica config.',
      updatedAt: nowIso(),
    })
    ...
    await updateDeploymentRecord(deploymentId, {
      ...
      lastMessage: 'Deployment completed. Multica is ready in the console.',
    ...
    await updateOrderDeploymentStatus(
      order.id,
      'deployed',
      'Deployment finished. Multica is live and visible in the console.',
    )
  } catch (error) {
...
async function upgradeMulticaInstance(order, targetVersion) {
  if (order.payment_status !== 'paid') {
    throw new HttpError(400, 'Pay for this order before upgrading Multica.')
  }
  ...
  if (!claw.workspace_path || !claw.runtime_user || !claw.service_name) {
    throw new HttpError(400, 'Multica runtime metadata is incomplete. Re-deploy the instance first.')
  }

  if (getAgentUpgradeStatus(claw) === 'in_progress') {
    throw new HttpError(409, 'Multica upgrade is already in progress.')
  }

  const normalizedVersion = String(targetVersion ?? '').trim()
  if (!normalizedVersion) {
    throw new HttpError(400, 'Choose a target Multica version first.')
  }
  ...
  await updateOrderDeploymentStatus(order.id, order.deployment_status, `Upgrading Multica to ${normalizedVersion}.`)
  ...
  try {
    ...
    await updateOrderDeploymentStatus(order.id, order.deployment_status, `Multica upgraded to ${normalizedVersion}.`)
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Multica upgrade failed.'
    await updateAgentUpgradeState(claw.id, {
    ...
    await updateOrderDeploymentStatus(order.id, order.deployment_status, `Multica upgrade failed: ${message}`)
    throw error
  }
...
async function refreshOrderDeploymentAfterInstanceRemoval(order) {
  ...
  if (latestDeployment) {
    await updateOrderDeploymentStatus(
      ...
      latestDeployment.last_message || 'Latest Multica record is available in the console.',
    )
    return
  }

  if (order.payment_status === 'paid') {
    await updateOrderDeploymentStatus(order.id, 'failed', 'Multica uninstalled. Create Multica to deploy again.')
    return
  }

  await updateOrderDeploymentStatus(order.id, 'pending_payment', 'Waiting for payment before Multica can be launched.')
}

async function stopMulticaInstance(order) {
  if (order.payment_status !== 'paid') {
    throw new HttpError(400, 'Pay for this order before managing Multica.')
  }
  ...
  await updateOrderDeploymentStatus(order.id, order.deployment_status, 'Multica stopped. You can still uninstall or redeploy it later.')
