...
NGINX_CONFIG="${NGINX_CONFIG:-/etc/nginx/sites-available/aigeamy.com}"
...
map $http_x_forwarded_proto $multica_forwarded_proto {
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/www.aigeamy.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/www.aigeamy.com/privkey.pem;
...
if [[ -f "$NGINX_CONFIG" ]]; then
  cp "$NGINX_CONFIG" "$BACKUP_ROOT/nginx-aigeamy.com"
fi

log "Updating production env without printing secrets"
set_env_var APP_ORIGIN "https://www.aigeamy.com,https://aigeamy.com"
...
if [[ ! -f /etc/letsencrypt/live/www.aigeamy.com/fullchain.pem ]]; then
