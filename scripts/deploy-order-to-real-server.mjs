...
import { loadDeploymentConfig } from '../server-lib/deployment-config.mjs'
...
const configPath = resolve(projectRoot, 'multica.config.json')
...
function buildInstanceName(ownerName, modelId, channelId) {
  const base = slugify(`${ownerName}-${modelId}-${channelId}`) || 'multica'
...
try {
  ...
  const instanceName = buildInstanceName(owner.name ?? 'multica-guest', order.model_id, order.channel_id)
  ...
  const statusMessage = hasSavedToken
    ? 'Multica console is ready. The saved channel token and model preset were written into the instance.'
    : 'Multica console is ready. The model preset was written into the instance; bind a channel token later when you are ready.'
