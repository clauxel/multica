...
export async function findRemotePackageTarget({ database, query }) {
  ...
  if (!normalizedQuery) {
    throw new Error('Missing instance query. Provide an instance name such as multica-guest-gpt-5-4-telegram-c8894e.')
  }
  ...
  if (!target) {
    throw new Error(`Unable to find a deployed Multica instance matching "${normalizedQuery}".`)
  }
...
if [ ! -d "$APP_PATH" ]; then
  echo "Multica app directory not found at $APP_PATH" >&2
  exit 1
fi
