...
function createOrder(overrides = {}) {
  return {
    ...
    statusMessage: 'Waiting for payment before Multica can be launched.',
...
function createDeployment(overrides = {}) {
  return {
    id: 'deployment-1',
    instanceName: 'Telegram Multica #1',
    status: 'deployed',
...
test('没有部署或实例的订单不会占用 Multica Management 列表', () => {
  const rows = buildMulticaManagementRows({
    orders: [createOrder()],
    claws: [],
  })
...
test('鏈€鏂伴儴缃叉鍦ㄨ繘琛屾椂鍙湁褰撳墠閭ｄ竴琛屾樉绀?Deployment running', () => {
  const failedDeployment = createDeployment({
    id: 'deployment-1',
    status: 'failed',
    triggerMode: 'manual',
