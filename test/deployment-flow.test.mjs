...
function createTestConfig(configPath, overrides = {}) {
  const baseConfig = {
    ...
    multica: {
      ...
      startCommand: 'multica gateway run --bind lan --port "$PORT" --token "$MULTICA_GATEWAY_TOKEN" --force',
...
function extractInjectedBootstrap(html) {
  ...
  assert.ok(match, 'Expected injected Multica bootstrap script.')
...
test('管理员只能删除自己名下的 Multica 实例，并且 console-data 只给自己实例删除权限', async () => {
  ...
  try {
    ...
    assert.equal(otherOrder?.canAdminDeleteMultica, false)
    ...
    assert.match(otherDeletePayload.message, /own account/i)
    ...
    assert.equal(deleteOwnResponse.message, 'Multica deleted successfully.')
...
test('部署配置文件在没有 server 段时仍可正常读取', () => {
  ...
  process.env.APP_ORIGIN = 'https://www.aigeamy.com,https://aigeamy.com'
...
test('读取 Multica 版本时不依赖 server 段', () => {
...
test('带有额外引号的 Multica 仓库配置会被规范化，避免克隆空仓库地址', () => {
...
test('生产部署示例配置包含有效的 Multica 仓库地址', () => {
...
test('默认 Multica 部署命令使用适合构建的 npm 配置', () => {
...
test('压缩包部署配置支持直接引用服务器上的预上传压缩包路径', () => {
  ...
  createTestConfig(configPath, {
    multica: {
      sourceType: 'archive',
      archivePath: '/root/multica-templates/multica-template.tar.gz',
      archiveUrl: '',
      installCommand: '',
      buildCommand: '',
    },
  ...
  assert.equal(config.multica.sourceType, 'archive')
  assert.equal(config.multica.archivePath, '/root/multica-templates/multica-template.tar.gz')
...
test('远端打包会优先复用当前 archivePath 并锁定实例 app 目录', () => {
...

  {
    ...
    assert.equal(request.archivePath, '/root/multica-templates/multica-template.tar.gz')
    ...
    assert.match(script, /ARCHIVE_PATH='\/root\/multica-templates\/multica-template\.tar\.gz'/)
    })
  }
})
...
test('启用实例路由入口后，SSH 部署计划会回写固定 console URL 并生成路由文件', () => {
  ...
  process.env.MULTICA_DEPLOY_USERNAME = 'root'
  process.env.MULTICA_DEPLOY_ROOT_PASSWORD = 'env-password'
  process.env.MULTICA_ROUTER_BASE_URL = 'http://10.128.0.4:19080'
  ...
  try {
    ...
    assert.equal(preview.plan.consoleUrl, 'http://10.128.0.4:19080/instances/demo-claw-router/')
    assert.equal(preview.plan.publicEndpoint, 'http://10.128.0.4:19080/instances/demo-claw-router/')
...
test('部署计划会生成只允许实例目录写入的 systemd 服务', () => {
  ...
  assert.match(preview.service, /ExecStart=.*multica gateway run --allow-unconfigured --bind lan --port "\$PORT" --token "\$MULTICA_GATEWAY_TOKEN" --force/)
})
...
test('部署计划只让 root 执行特权步骤，应用目录与配置写入由实例用户完成', () => {
  ...
  assert.match(preview.script, /Multica service \$SERVICE_NAME did not become active/)
  assert.match(preview.script, /CONSOLE_READY=0/)
  assert.match(preview.script, /Multica console on port \$CONSOLE_PORT did not become reachable\./)
...
test('部署计划支持通过压缩包 URL 快速下发 Multica', () => {
...
test('部署计划支持直接解压服务器上预上传的 Multica 压缩包', () => {
  ...
  assert.match(preview.script, /ARCHIVE_SOURCE_PATH='\/root\/multica-templates\/multica-template\.tar\.gz'/)
...
test('部署计划会把 dev 或 prod PostgreSQL 连接写入 Multica 实例环境变量', () => {
  ...
  process.env.MULTICA_POSTGRES_USER = 'multica_dev'
  ...
  try {
    ...
    assert.equal(
      ...
      'postgresql://multica_dev:dev-db-password@127.0.0.1:5432/multica_dev',
    )
  } finally {
    for (const [key, value] of Object.entries(previousEnvironment)) {
      if (value === undefined) {
...
test('已支付订单会自动部署，支持手动触发与版本升级', async () => {
  ...
  try {
    ...
    assert.equal(uninstallResponse.message, 'Multica deleted successfully.')
...
test('无 token 也能完成部署并打开 Multica 控制台', async () => {
...
test('部署失败但已知实例名时仍可卸载并清理 mock 目录', async () => {
  ...
  try {
    ...
    assert.equal(response.message, 'Multica uninstalled successfully.')
...
test('控制台代理会保留浏览器原始 origin 和 referer 给上游实例', async () => {
  ...
  try {
    ...
    const rootConsoleUrl = new URL(tokenizedConsole.url, baseUrl)
    const browserOrigin = 'https://www.aigeamy.com'
    const browserReferer = 'https://www.aigeamy.com/console'
...
test('SSH 部署模板会继承本机 Multica 模型配置，并把 auth profiles 同步到 state 目录', () => {
  ...
  try {
    ...
    assert.deepEqual(renderedConfig.gateway.controlUi.allowedOrigins, ['https://www.aigeamy.com', 'https://aigeamy.com'])
...
test('SSH 部署模板在缺少本机 Multica 模板时会用 env 生成模型代理配置', () => {
  ...
  try {
    ...
    assert.equal(
      ...
      'http://127.0.0.1:5173/api/internal/model-proxy/demo-claw-generated-proxy/v1',
    )
    ...
    process.env.MULTICA_MODEL_PROXY_INTERNAL_BASE_URL = 'http://10.128.0.2:5173/api/internal/model-proxy'
    ...
    assert.equal(
      ...
      'http://10.128.0.2:5173/api/internal/model-proxy/demo-claw-generated-proxy-private-base/v1',
    )
  } finally {
    for (const [key, value] of Object.entries(previousEnvironment)) {
      if (value === undefined) {
